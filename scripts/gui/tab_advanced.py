...
class TabAdvanced:
    ...
    def _build_audio(self, parent):
        ...
        if buf_cur not in buf_vals:
            ...
            self._bufsize_var.set(str(buf_cur))
        self._combo_row(lf, "Bufor audio", "setbufsize", buf_vals, buf_cur,
                        "Rozmiar bufora FFT — potęga 2\n"
                        "Większy = więcej 'grawitacji'\n"
                        "512=szybki, 4096=ciężki")

        # Rozmiar próbki — combobox potęgi 2
        ...
        # próbka nie może być większa niż bufor
        ...
        if smp_cur not in smp_vals:
            ...
            self._samplesize_var.set(str(smp_cur))
        self._combo_row(lf, "Rozmiar próbki", "setsamplesize", smp_vals, smp_cur,
                        "256=172UPS 512=86UPS 1024=43UPS 2048=21UPS\n"
                        "Zawsze <= bufor audio")

        # Częstotliwość próbkowania — combobox
        ...
        if rate_cur not in rate_vals:
            ...
            self._samplerate_var.set(str(rate_cur))
        self._combo_row(lf, "Częst. próbkowania", "setsamplerate", rate_vals, rate_cur,
        ...
        tk.Label(fps_row, text="Limit FPS", font=("Arial", 8),
        ...
        tk.Label(fps_row, text="fps", font=("Arial", 8),
    ...
    def _write_request(self, key, val):
        ...
        with open(RC_GLSL, "w") as f: f.write(new)
    ...
    def _build_glava_flags(self, parent):
        ...
        tk.Label(lf, text=T.get("label_extra_flags", "Dodatkowe flagi:"),
        ...
        tk.Label(lf, text=T.get("label_flags_note", "np. --desktop --force-mod=bars"),
